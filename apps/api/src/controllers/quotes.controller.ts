...
export class QuotesController {
  ...
  @Get() async list(@Req() request: Request) { await this.auth.requireUser(request, "crm:read"); return { quotes: await this.quotes.list() }; }
  ...
  @Get(":id") async get(@Req() request: Request, @Param("id") id: string) {
    await this.auth.requireUser(request, "crm:read"); return { quote: await this.quotes.get(id) };
  }
  ...
  @Delete(":id") async archive(@Req() request: Request, @Param("id") id: string) {
    const user = await this.auth.requireUser(request, "crm:write"); return { quote: await this.quotes.archive(id, user) };
  }
  @Post(":id/convert")
  @HttpCode(200)
