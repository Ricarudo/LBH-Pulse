...
WITH normalized_clients AS (
  SELECT
    "id",
    REGEXP_REPLACE(LOWER(TRIM("displayName")), '[^a-z0-9]+', '', 'g') AS normalized_name,
    COUNT(*) OVER (
      PARTITION BY REGEXP_REPLACE(LOWER(TRIM("displayName")), '[^a-z0-9]+', '', 'g')
    ) AS match_count
  FROM "Client"
...
WHERE request."clientId" IS NULL
  AND request."archivedAt" IS NULL
  AND client.match_count = 1
...
WHERE quote."clientId" IS NULL
  AND request."relatedQuoteId" = quote."id"
