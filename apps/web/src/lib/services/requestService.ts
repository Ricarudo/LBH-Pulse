...
async function findChecklistTemplates(
  ...
  const templates = await tx.requestChecklistTemplate.findMany({
    where: {
      active: true,
      archivedAt: null,
      OR: [{ key: "general" }, { serviceCategory: { in: serviceCategories } }, { requestType }]
    },
    include: { items: { where: { active: true }, orderBy: { sortOrder: "asc" } } }
  });
  return templates.map((template) => ({
    template,
    matchType: template.key === "general" ? "CORE" : template.serviceCategory ? "TRADE" : "REQUEST_TYPE",
    matchValue: template.serviceCategory ?? template.requestType
  })).sort((left, right) => {
    const order: Record<string, number> = { CORE: 0, TRADE: 1, REQUEST_TYPE: 2 };
    return order[left.matchType] - order[right.matchType] || (left.matchValue ?? "").localeCompare(right.matchValue ?? "");
  });
}
...
async function reconcileRequestChecklists(
  ...
  for (const match of desired) {
    ...
    await tx.requestChecklistInstance.create({
      data: {
        requestId, templateId: match.template.id, templateKeySnapshot: match.template.key,
        templateNameSnapshot: match.template.name, matchType: match.matchType, matchValue: match.matchValue,
        items: { create: buildChecklistItemCreateData(match.template).map((item) => ({ ...item, requestId })) }
      }
    });
  }
}
    ...
    for (const match of matches) {
      await tx.requestChecklistInstance.create({
        data: {
          ...
          items: { create: buildChecklistItemCreateData(match.template).map((item) => ({ ...item, requestId: created.id })) }
        }
      });
    }
    const hydrated = await tx.request.findUniqueOrThrow({ where: { id: created.id }, include: requestInclude });
...
export async function updateRequest(
  ...
  const now = new Date();
  const requestedCategories = input.serviceCategories ?? (input.serviceCategory ? [input.serviceCategory] : undefined);
  const checklistRulesChanged = requestedCategories !== undefined || input.requestType !== undefined;
  if (checklistRulesChanged && existingRequest.status === "Converted to Quote") throw new Error("REQUEST_CONVERTED_LOCKED");
  if (checklistRulesChanged) {
    const nextCategories = requestedCategories ?? (existingRequest.trades.length ? existingRequest.trades.map((trade) => trade.serviceCategory) : [existingRequest.serviceCategory]);
    ...
    await prisma.$transaction(async (tx) => {
      if (requestedCategories) {
        ...
        await tx.requestTrade.createMany({ data: nextCategories.map((serviceCategory) => ({ requestId: id, serviceCategory })) });
      }
...
export async function updateRequestChecklistItem(
  ...
  if (currentRequest.status === "Converted to Quote") throw new Error("REQUEST_CONVERTED_LOCKED");
  ...
  const existingItem = await prisma.requestChecklistItem.findFirst({
    where: {
      id: itemId,
      requestId: id,
      OR: [{ checklistInstanceId: null }, { checklistInstance: { active: true } }]
    }
  });
