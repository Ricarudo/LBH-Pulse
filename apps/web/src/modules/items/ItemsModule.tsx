          ...
          <table className="data-table items-table">
            ...
            <tbody>
              {filteredItems.map((item) => (
                <tr key={item.id}>
                  <td>
                    ...
                    <br />
                    <span className="table-muted">{item.description || item.quoteDescription || "No description"}</span>
                  </td>
                  <td>{item.itemType}</td>
                  <td>
                    <strong>{item.sku || "No SKU"}</strong>
                    <br />
                    <span className="table-muted">{item.partNumber || "No part number"}</span>
                  </td>
                  ...
                  <td>
                    <div className="items-row-actions">
                      <button className="toolbar-button compact" type="button" onClick={() => openEdit(item)} disabled={!canWrite}>
                        <Pencil size={15} />
                        Edit
                      </button>
                      {item.status === "ACTIVE" ? (
                        <button className="toolbar-button compact danger" type="button" onClick={() => void markInactive(item)} disabled={!canWrite}>
                          Inactive
                        </button>
                      ) : null}
                    </div>
              ...
              {!filteredItems.length ? (
                <tr><td colSpan={9}><div className="work-queue-state"><Boxes size={20} />No items match this view.</div></td></tr>
              ) : null}
            </tbody>
          </table>
        ) : null}
