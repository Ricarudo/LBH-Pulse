...
export function SettingsChecklistsSection({ readOnly = false }: { readOnly?: boolean }) {
  ...
  return (
    <div className="checklist-library">
      <aside className="checklist-library-sidebar">
        ...
        <button className="primary-button full-width" onClick={() => void createTemplate()} disabled={busy || readOnly}><Plus size={17} />New template</button>
          ...
          <>
            <div className="checklist-editor-header">
              ...
              <div className="workspace-actions">
                {!draft.archivedAt ? <button className="toolbar-button compact" disabled={readOnly} onClick={() => void action("duplicate")}><Copy size={16} />Duplicate</button> : null}
                {draft.archivedAt ? <button className="toolbar-button compact" disabled={readOnly} onClick={() => void action("restore")}>Restore</button> : draft.key !== "general" ? <button className="toolbar-button compact danger-button" disabled={readOnly} onClick={() => void action("archive")}><Archive size={16} />Archive</button> : null}
              </div>
            </div>
            ...
            <div className="checklist-item-editor-list">
              {ordered(draft.items).map((item, index) => <article className={item.active ? "checklist-item-editor" : "checklist-item-editor inactive"} key={item.id || `new-${index}`}>
                <div className="checklist-reorder"><button aria-label={`Move ${item.label} up`} disabled={readOnly || index === 0 || Boolean(draft.archivedAt)} onClick={() => moveItem(index, -1)}><ArrowUp size={15} /></button><button aria-label={`Move ${item.label} down`} disabled={readOnly || index === draft.items.length - 1 || Boolean(draft.archivedAt)} onClick={() => moveItem(index, 1)}><ArrowDown size={15} /></button></div>
                <div className="settings-form checklist-item-fields">
                  <label className="wide"><span>Item label</span><input value={item.label} disabled={readOnly || Boolean(draft.archivedAt)} onChange={(event) => updateItem(index, { label: event.target.value })} /></label>
                  <label><span>Group</span><input value={item.group} disabled={readOnly || Boolean(draft.archivedAt)} onChange={(event) => updateItem(index, { group: event.target.value })} /></label>
                  <label className="wide"><span>Description</span><input value={item.description} disabled={readOnly || Boolean(draft.archivedAt)} onChange={(event) => updateItem(index, { description: event.target.value })} /></label>
                  <label><span>Condition</span><select value={item.appliesWhen} disabled={readOnly || Boolean(draft.archivedAt)} onChange={(event) => updateItem(index, { appliesWhen: event.target.value })}><option value="">Always</option><option value="siteVisitRequired">Site visit required</option></select></label>
                </div>
                <div className="checklist-item-toggles"><label><input type="checkbox" checked={item.required} disabled={readOnly || Boolean(draft.archivedAt)} onChange={(event) => updateItem(index, { required: event.target.checked })} />Required</label><label><input type="checkbox" checked={item.active} disabled={readOnly || Boolean(draft.archivedAt)} onChange={(event) => updateItem(index, { active: event.target.checked })} />Active</label></div>
              </article>)}
            </div>
            <div className="sticky-save-bar"><p className="settings-inline-message" aria-live="polite">{message || (readOnly ? "Your role has view-only access." : dirty ? "Unsaved changes" : "")}</p>{!draft.archivedAt ? <button className="primary-button" disabled={readOnly || !dirty || busy} onClick={() => void save()}><CheckCircle2 size={17} />{busy ? "Saving…" : "Save template"}</button> : null}</div>
          </>
