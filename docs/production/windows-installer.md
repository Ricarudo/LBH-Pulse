...
`Pulse-Setup-0.1.2.exe` is the normal installation and the supported `0.1.0`/`0.1.1` update path for a Windows operator. It installs the existing production Docker Compose deployment; it is not a desktop wrapper. The operator does not need the Pulse source, Git, Node.js, or npm.
...
5. Upgrade representative `0.1.0` and `0.1.1` installations with `0.1.2`, rerun `0.1.2` for repair, then test a staged newer manifest and verified pre-update backup.
